...
# Overlap-merging double-Gaussian case for "line_ratio is None". 
# If +/- sigma_factor * sigma windows of the two peaks overlap, treat as one blended line instead of choosing closest peak
merge_overlapping_peaks = True

# SNR threshold for secondary peak to be considered as a "Double Gaussian" fit. 
min_second_peak_snr = 0.0
# If threshold is not met, refitted as single gaussian.
downgraded_from_double = True
...
def pbres(pbfile, fractions=(0.95, 0.50)):
    ...
    if pb_data.ndim == 4:
        ...
        pb_2d = pb_data[0, :, :]  # if shape = (chan, y, x)
    elif pb_data.ndim == 2:
        pb_2d = pb_data
    else:
        raise ValueError(f"Unexpected PB dimensions: {pb_data.shape}")
...
def rms_noise(imagename, pbfile, bkg_radius, num_regions=50, seed=42, diagnostic=None):
    import numpy as np
    np.random.seed(seed)
    ...
    for i in range(num_regions):
        for j in range(10):
            ...
            if not coord_bounds(ra_bkg, dec_bkg, ra_min, ra_max, dec_min, dec_max):
                continue
            ...
            try:
                ...
                stats_result = imstat(imagename=imagename, region=region, axes=[0, 1])
    ...
    if diagnostic:
        ...
        plt.hist(flat_flux_spectra, bins=50, density=True, alpha=0.7, color='steelblue', edgecolor='black')

        mu, sigma = np.mean(flat_flux_spectra), np.std(flat_flux_spectra)
        x = np.linspace(flat_flux_spectra.min(), flat_flux_spectra.max(), 200)
        plt.plot(x, stats.norm.pdf(x, mu, sigma), color='maroon', lw=2.5, label=f'μ={mu:.2e}\nσ={sigma:.2e}')
...
def plot_aperture(data, wcs, mom_mask, center_pix, r_pix, r_arcsec):
    ...
    ax.contour(mom_mask.astype(int), levels=[0.5], colors='cyan', linewidths=0.8)
    cx, cy = center_pix
...
def measurement(imagename, region):
    ...
    frequencies = [coords.toworld([0, 0, freq, 0])['numeric'][2] for freq in range(num_channels)]  # RA, DEC, FREQ, STOKES
    im.close()
...
def intervals_overlap(low1, high1, low2, high2):
    """True if [low1, high1] and [low2, high2] intersect."""
...
for i in range(len(id)):
    ...
    for line in lines_to_process:
        ...
        if aperture_selection == 'auto':
            ...
            raise ValueError(f"Invalid aperture_selection: {aperture_selection}. Use 'auto' or a numeric value in arcsec")
...
for ln, fnames in fits_files.items():
    ...
    for f, pbfile, nopbcor_file in zip(fnames, pb_list[field][ln], noPBcor_list[field][ln]):
        if nopbcor_file.endswith(".fits"):
            nopbcor_imagename = nopbcor_file.replace(".fits", ".image")
            importfits(fitsimage=nopbcor_file, imagename=nopbcor_imagename, overwrite=True)
        else:
            nopbcor_imagename = nopbcor_file

        if rms_diagnose:
            diagnose_file = f"RMS_Diagnostic_{ln.replace(' ', '_').replace('&', 'and')}_r{radius}_{timestamp}.pdf"
        ...
        if not np.isfinite(rms_med) or rms_med <= 0:
            continue
        for ln2 in split_lines:
            rms_chan_dict[ln2] = rms_chan

        if f.endswith(".fits"):
            flux_imagename = f.replace(".fits", ".image")
            importfits(fitsimage=f, imagename=flux_imagename, overwrite=True)
        else:
            flux_imagename = f
        ...
        for tid, ra_deg, dec_deg in zip(id, ra, dec):
            ...
            if not coord_bounds(ra_deg, dec_deg, ra_min, ra_max, dec_min, dec_max):
                continue
...
for (region_label, line), region_file in region_id_map.items():
    ...
    for i, fits_f in enumerate(fits_file, start=1):
        ...
        try:
            ...
            if line_ratio is not None:
                ...
                try:
                    popt2, pcov2 = curve_fit(
                        two_gaussian_free, frequencies, fluxes,
                        p0=[amp1, mean1, guess_std, amp2, guess_mean2, guess_std],
                        bounds=([0, min(frequencies), 0, 0, min(frequencies), 0],
                                [np.inf, max(frequencies), np.inf, np.inf, max(frequencies), np.inf]),
                        maxfev=10000
                    )
            ...
            if best_label == 'Single Gaussian':
                ...
                immoments(imagename=imagename, moments=[0, 1, 2], axis='spectral', region=region_file, chans=chans1,
                          outfile=str(dirs["finalmommap"] / f"{region_label}_spw{spw_label}_{line}_G1_{mom_reg_size}arcsec_{timestamp}"))
                ...
                immoments(imagename=imagename, moments=[0, 1, 2], axis='spectral', region=region_file, chans=chans1,
                          outfile=str(dirs["finalmommap"] / f"{region_label}_spw{spw_label}_{line}_G1_{mom_reg_size}arcsec_{timestamp}"))
                immoments(imagename=imagename, moments=[0, 1, 2], axis='spectral', region=region_file, chans=chans2,
                          outfile=str(dirs["finalmommap"] / f"{region_label}_spw{spw_label}_{line}_G2_{mom_reg_size}arcsec_{timestamp}"))
            ...
            for li, lines in enumerate(lines_split):
                ...
                if np.min(freq_GHz) <= rest_frequencies_z <= np.max(freq_GHz):
                    plt.axvline(x=rest_frequencies_z, color=color[li], linestyle='--',
                                label=f'{lines} at $z_{{\\mathrm{{SED}}}}$ = {region_z_phot_map[region_label]:.3f}')
    ...
    # Visual diagnostic for overlapping regions
    '''
    if best_fit and best_label == "Single Gaussian":
        lower_bound1 = mean1 - abs(2 * std1)
        upper_bound1 = mean1 + abs(2 * std1)
        ax = plt.gca()

        ax.axvspan(lower_bound1 / 1e9, upper_bound1 / 1e9, color='red', alpha=0.2, zorder=0)

        ax.axvline(lower_bound1 / 1e9, color='red', ls='--')
        ax.axvline(upper_bound1 / 1e9, color='red', ls='--')

    if best_fit and best_label == "Double Gaussian":
        lower_bound1 = mean1 - abs(2 * std1)
        upper_bound1 = mean1 + abs(2 * std1)

        lower_bound2 = mean2 - abs(2 * std2)
        upper_bound2 = mean2 + abs(2 * std2)
        ax = plt.gca()

        # Peak 1
        ax.axvspan(lower_bound1 / 1e9, upper_bound1 / 1e9, color='tab:red', alpha=0.20, zorder=0)
        ax.axvline(lower_bound1 / 1e9, color='tab:red', ls='--')
        ax.axvline(upper_bound1 / 1e9, color='tab:red', ls='--')

        # Peak 2
        ax.axvspan(lower_bound2 / 1e9, upper_bound2 / 1e9, color='tab:blue', alpha=0.20, zorder=0)
        ax.axvline(lower_bound2 / 1e9, color='tab:blue', ls='--')
        ax.axvline(upper_bound2 / 1e9, color='tab:blue', ls='--')
    '''
    ...
    if best_fit:
        ...
        if best_label == 'Double Gaussian' and line_ratio is None:
            _second_peak_rms = mean_rms_dict[lines_split[0]][region_label] / 1e3
            second_peak_snr = best_amp2 / _second_peak_rms if _second_peak_rms else 0
            if second_peak_snr < min_second_peak_snr:
                effective_label = 'Single Gaussian'
                downgraded_from_double = True
                print(f"Second peak SNR={second_peak_snr:.2f} < {min_second_peak_snr}: "
                      f"treating as noise, reporting as single Gaussian instead of merging.")

        if effective_label == 'Single Gaussian':
            ...
            best_index = rng.choice(min_index) if len(min_index) > 1 else min_index[0]
            best_line = line_name[best_index]

            profile_type[f"{region_label}"][f"{best_line}"] = (
                "Single Gaussian (downgraded from double; 2nd peak below SNR threshold)"
                if downgraded_from_double else "Single Gaussian"
            )
            ...
            for i in range(niter):
                noise_flux = best_fluxes + np.random.normal(0, rms_chan_dict[best_line], size=fluxes.shape)

                try:
                    popt1_noise, pcov1_noise = curve_fit(one_gaussian, best_freqs[mask1], noise_flux[mask1],
                                                          p0=[best_amp1, best_mean1, best_std1], maxfev=10000)
                    ...
                    gauss_lum1_val = 3.25 * 10 ** 7 * (dl1 ** 2 / ((1 + calc_z1) * (rest_frequencies[best_line]) ** 2)) * gaussian_flux_vel1
                    int_lum1_val = 3.25 * 10 ** 7 * (dl1 ** 2 / ((1 + calc_z1) * (rest_frequencies[best_line]) ** 2)) * integrated_flux_vel1

                    gaussian_fluxes1.append(gaussian_flux_vel1)
                    gaussian_lum1.append(gauss_lum1_val)
                    if not np.isnan(integrated_flux_vel1):
                        integrated_fluxes1.append(integrated_flux_vel1)
                        integrated_lum1.append(int_lum1_val)
            ...
            if params.ndim == 2 and params.shape[0] > params.shape[1]:
                fig = corner.corner(params, labels=labels, show_titles=True, title_fmt=".3f",
                                     quantiles=[0.25, 0.5, 0.75], title_kwargs={"fontsize": 10})
            else:
        ...
        elif effective_label == 'Double Gaussian':
            ...
            # do the +/- sigma_factor*sigma windows of the two components overlap?
            peaks_overlap = intervals_overlap(lower_bound1, upper_bound1, lower_bound2, upper_bound2)
            ...
            if line_ratio is not None:
                ...
                best_index1 = rng.choice(min_index1) if len(min_index1) > 1 else min_index1[0]
                ...
                best_index2 = rng.choice(min_index2) if len(min_index2) > 1 else min_index2[0]
                best_line2 = line_name[best_index2]

                profile_type[f"{region_label}"][f"{best_line1}"] = "Double Gaussian (two known lines)"
                profile_type[f"{region_label}"][f"{best_line2}"] = "Double Gaussian (two known lines)"

            elif merge_overlapping_peaks and peaks_overlap:
                merged_case = True

                expected_freq = [rest_frequencies[line] / (1 + region_z_phot_map[region_label]) for line in line_name]
                centroid_freq_est = (best_amp1 * best_mean1 + best_amp2 * best_mean2) / (best_amp1 + best_amp2)
                distance = [abs(centroid_freq_est - expected * 1e9) for expected in expected_freq]
                min_index = [i for i, d in enumerate(distance) if d == min(distance)]
                best_index = rng.choice(min_index) if len(min_index) > 1 else min_index[0]
                best_line = line_name[best_index]
                best_line1 = best_line2 = None

                profile_type[f"{region_label}"][f"{best_line}"] = "Double Gaussian (blended, merged)"
                ...
                candidates = []
                for ii, expected in enumerate(expected_freq):
                    expected_Hz = expected * 1e9
                    distances_peak1 = abs(best_mean1 - expected_Hz)
                    distances_peak2 = abs(best_mean2 - expected_Hz)

                    candidates.append((distances_peak1, "peak1", line_name[ii]))
                    candidates.append((distances_peak2, "peak2", line_name[ii]))
                ...
                profile_type[f"{region_label}"][f"{best_line}"] = "Double Gaussian (separated)"
            ...
            if merged_case:
                grid_span = 5 * max(best_std1, best_std2)
                grid_lo = min(best_mean1, best_mean2) - grid_span
                grid_hi = max(best_mean1, best_mean2) + grid_span
                eval_grid = np.linspace(grid_lo, grid_hi, 4000)

                for i in range(niter):
                    noise_flux = best_fluxes + np.random.normal(0, rms_chan_dict[best_line], size=fluxes.shape)

                    try:
                        popt2_noise, pcov2_noise = curve_fit(
                            two_gaussian_free, best_freqs[mask], noise_flux[mask],
                            p0=[best_amp1, best_mean1, best_std1, best_amp2, best_mean2, best_std2],
                            maxfev=10000
                        )
                        amp1_n, mean1_n, std1_n, amp2_n, mean2_n, std2_n = popt2_noise

                        # Flux-weighted centroid frequency
                        mean_merged = (amp1_n * mean1_n + amp2_n * mean2_n) / (amp1_n + amp2_n)

                        # Combined Gaussian line flux (both components)
                        gflux1 = amp1_n * std1_n * math.sqrt(2.0 * math.pi) * erf(sigma_factor / math.sqrt(2.0))
                        gflux2 = amp2_n * std2_n * math.sqrt(2.0 * math.pi) * erf(sigma_factor / math.sqrt(2.0))
                        gaussian_flux_vel_merged = (gflux1 + gflux2) * (3e5 / mean_merged)

                        # FWHM of combined components
                        curve_n = two_gaussian_free(eval_grid, amp1_n, mean1_n, std1_n, amp2_n, mean2_n, std2_n)
                        peak_val = curve_n.max()
                        half_val = peak_val / 2.0
                        above_half = eval_grid[curve_n >= half_val]
                        if above_half.size < 2:
                            continue
                        fwhm_merged_freq = above_half.max() - above_half.min()
                        fwhm_merged_vel = fwhm_merged_freq * (3e5 / mean_merged)

                        # Effective sigma of blended profile
                        sigma_eff = fwhm_merged_freq / (2 * np.sqrt(2 * np.log(2)))
                        lower_eff = mean_merged - sigma_factor * sigma_eff
                        upper_eff = mean_merged + sigma_factor * sigma_eff
                        mask_eff = (best_freqs >= lower_eff) & (best_freqs <= upper_eff)

                        # Integrated flux over this effective window
                        valid = ~np.isnan(noise_flux[mask_eff])
                        if best_freqs[0] > best_freqs[-1]:
                            best_freqs = best_freqs[::-1]
                        freqs_masked = best_freqs[mask_eff][valid]
                        flux_masked = noise_flux[mask_eff][valid]
                        if freqs_masked.size < 2:
                            continue
                        order = np.argsort(freqs_masked)
                        integrated_flux_merged = np.trapz(flux_masked[order], freqs_masked[order])
                        integrated_flux_vel_merged = integrated_flux_merged * (3e5 / mean_merged)

                        # Single merged redshift
                        calc_z_merged = (rest_frequencies[best_line] / (mean_merged / 1e9)) - 1

                        # Luminosity from the merged flux + merged redshift
                        dl_merged = cosmo.luminosity_distance(calc_z_merged).value
                        gauss_lum_merged = 3.25 * 10 ** 7 * (dl_merged ** 2 / ((1 + calc_z_merged) * (rest_frequencies[best_line]) ** 2)) * gaussian_flux_vel_merged
                        int_lum_merged = 3.25 * 10 ** 7 * (dl_merged ** 2 / ((1 + calc_z_merged) * (rest_frequencies[best_line]) ** 2)) * integrated_flux_vel_merged

                        flux_density1.append(amp1_n + amp2_n)
                        mean1_list.append(mean_merged)
                        gaussian_fluxes1.append(gaussian_flux_vel_merged)
                        gaussian_lum1.append(gauss_lum_merged)
                        if not np.isnan(integrated_flux_vel_merged):
                            integrated_fluxes1.append(integrated_flux_vel_merged)
                            integrated_lum1.append(int_lum_merged)
                        fwhm1.append(fwhm_merged_vel)
                        z1.append(calc_z_merged)

                    except RuntimeError:
                        continue

                # Aggregate and store the single merged estimate under best_line
                gaussian_fluxes1 = np.array(gaussian_fluxes1)
                gaussian_flux1_mean = np.mean(gaussian_fluxes1)
                gaussian_flux1_std = np.std(gaussian_fluxes1)
                ...
                gaussian_lum1 = np.array(gaussian_lum1)
                gauss_lum[f"{region_label}"][f"{best_line}"] = np.mean(gaussian_lum1)
                gauss_lum_err[f"{region_label}"][f"{best_line}"] = np.std(gaussian_lum1)

                integrated_lum1 = np.array(integrated_lum1)
                int_lum[f"{region_label}"][f"{best_line}"] = np.mean(integrated_lum1)
                int_lum_err[f"{region_label}"][f"{best_line}"] = np.std(integrated_lum1)

                fwhm1 = np.array(fwhm1)
                line_width[f"{region_label}"][f"{best_line}"] = np.mean(fwhm1)
                line_width_err[f"{region_label}"][f"{best_line}"] = np.std(fwhm1)

                z1 = np.array(z1)
                region_line_z[f"{region_label}"][f"{best_line}"] = np.mean(z1)
                region_line_z_err[f"{region_label}"][f"{best_line}"] = np.std(z1)

                params_merged = np.array([gaussian_fluxes1, integrated_fluxes1, fwhm1, z1]).T
                labels_merged = ["Gaussian Flux (mJy km/s)", "Integrated Flux (mJy km/s)", "FWHM (km/s)", "z"]
                if params_merged.ndim == 2 and params_merged.shape[0] > params_merged.shape[1]:
                    fig = corner.corner(params_merged, labels=labels_merged, show_titles=True, title_fmt=".3f",
                                         quantiles=[0.25, 0.5, 0.75], title_kwargs={"fontsize": 10})
                else:
                    print("Skipping corner plot")
                    fig = None
                ...
                for i in range(niter):
                    if best_line1:
                        noise_flux = best_fluxes + np.random.normal(0, rms_chan_dict[best_line1], size=fluxes.shape)
                    else:
                        noise_flux = best_fluxes + np.random.normal(0, rms_chan_dict[best_line2], size=fluxes.shape)
                    try:
                        if line_ratio is not None:
                            popt2_noise, pcov2_noise = curve_fit(
                                lambda x, a1, m1, s1, a2, s2: two_gaussian(x, a1, m1, s1, a2, s2, line_ratio),
                                best_freqs[mask], noise_flux[mask],
                                p0=[best_amp1, best_mean1, best_std1, best_amp2, best_std2], maxfev=10000
                            )
                            ...
                            popt2_noise, pcov2_noise = curve_fit(
                                two_gaussian_free, best_freqs[mask], noise_flux[mask],
                                p0=[best_amp1, best_mean1, best_std1, best_amp2, best_mean2, best_std2], maxfev=10000
                            )
                        ...
                        if best_line1 is not None and mean1_noise is not None:
                            ...
                            dl1 = cosmo.luminosity_distance(calc_z1).value
                            gauss_lum1_val = 3.25 * 10 ** 7 * (dl1 ** 2 / ((1 + calc_z1) * (rest_frequencies[best_line1]) ** 2)) * gaussian_flux_vel1
                            int_lum1_val = 3.25 * 10 ** 7 * (dl1 ** 2 / ((1 + calc_z1) * (rest_frequencies[best_line1]) ** 2)) * integrated_flux_vel1
                        ...
                        if best_line2 is not None and mean2_noise is not None:
                            ...
                            gauss_lum2_val = 3.25 * 10 ** 7 * (dl2 ** 2 / ((1 + calc_z2) * (rest_frequencies[best_line2]) ** 2)) * gaussian_flux_vel2
                            int_lum2_val = 3.25 * 10 ** 7 * (dl2 ** 2 / ((1 + calc_z2) * (rest_frequencies[best_line2]) ** 2)) * integrated_flux_vel2

                            gaussian_fluxes2.append(gaussian_flux_vel2)
                            gaussian_lum2.append(gauss_lum2_val)
                            if not np.isnan(integrated_flux_vel2):
                                integrated_fluxes2.append(integrated_flux_vel2)
                                integrated_lum2.append(int_lum2_val)
                ...
                # Peak 1 aggregation
                if best_line1 is not None and len(gaussian_fluxes1) > 0:
                    gaussian_fluxes1 = np.array(gaussian_fluxes1)
                    gaussian_flux1_mean = np.mean(gaussian_fluxes1)
                    gaussian_flux1_std = np.std(gaussian_fluxes1)
                    ...
                    gaussian_lum1 = np.array(gaussian_lum1)
                    gauss_lum[f"{region_label}"][f"{best_line1}"] = np.mean(gaussian_lum1)
                    gauss_lum_err[f"{region_label}"][f"{best_line1}"] = np.std(gaussian_lum1)

                    integrated_lum1 = np.array(integrated_lum1)
                    int_lum[f"{region_label}"][f"{best_line1}"] = np.mean(integrated_lum1)
                    int_lum_err[f"{region_label}"][f"{best_line1}"] = np.std(integrated_lum1)

                    fwhm1 = np.array(fwhm1)
                    line_width[f"{region_label}"][f"{best_line1}"] = np.mean(fwhm1)
                    line_width_err[f"{region_label}"][f"{best_line1}"] = np.std(fwhm1)

                    z1 = np.array(z1)
                    region_line_z[f"{region_label}"][f"{best_line1}"] = np.mean(z1)
                    region_line_z_err[f"{region_label}"][f"{best_line1}"] = np.std(z1)
                    ...
                    if params1.ndim == 2 and params1.shape[0] > params1.shape[1]:
                        fig = corner.corner(params1, labels=labels1, show_titles=True, title_fmt=".3f",
                                             quantiles=[0.25, 0.5, 0.75], title_kwargs={"fontsize": 10})
                    else:
                        print("Skipping corner plot")
                        fig = None
                    ...
                    print(f"Peak 1 not a real component for {region_label} ({line}) -- skipping")

                # Peak 2 aggregation
                if best_line2 is not None and len(gaussian_fluxes2) > 0:
                    ...
                    gauss_lum[f"{region_label}"][f"{best_line2}"] = np.mean(gaussian_lum2)
                    gauss_lum_err[f"{region_label}"][f"{best_line2}"] = np.std(gaussian_lum2)

                    integrated_lum2 = np.array(integrated_lum2)
                    int_lum[f"{region_label}"][f"{best_line2}"] = np.mean(integrated_lum2)
                    int_lum_err[f"{region_label}"][f"{best_line2}"] = np.std(integrated_lum2)

                    fwhm2 = np.array(fwhm2)
                    line_width[f"{region_label}"][f"{best_line2}"] = np.mean(fwhm2)
                    line_width_err[f"{region_label}"][f"{best_line2}"] = np.std(fwhm2)

                    z2 = np.array(z2)
                    region_line_z[f"{region_label}"][f"{best_line2}"] = np.mean(z2)
                    region_line_z_err[f"{region_label}"][f"{best_line2}"] = np.std(z2)
                    ...
                    if params2.ndim == 2 and params2.shape[0] > params2.shape[1]:
                        fig = corner.corner(params2, labels=labels2, show_titles=True, title_fmt=".3f",
                                             quantiles=[0.25, 0.5, 0.75], title_kwargs={"fontsize": 10})
                    else:
                        print("Skipping corner plot")
                        fig = None
                    ...
                    print(f"Peak 2 not a real component for {region_label} ({line}) -- skipping")
        ...
        for i in range(len(id)):
            ...
            for line in line_name:
                ...
                row.extend([
                    ...
                    profile_type.get(str(id[i]), {}).get(line, 'ND'),
